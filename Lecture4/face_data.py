...
while True:
	ret, frame = cap.read()
	if ret == False: #executes until the frame is captured
		continue
	#once the frame is captured then->
	# Convert frame to grayscale
	gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)

	# Detect multi faces in the image
	faces = face_cascade.detectMultiScale(gray, 1.3, 5)#1.3 is scale and 5 is the quality factor
	k = 1
